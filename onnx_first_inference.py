...
def onnx_run_first_half(onnx_file, inputData, saveOutput = False, EP_list = CPU_EP_list, device = None, profiling = False, xml_file = None, singleLayerProfiling = False):
  '''
  ...
  #if singleLayerProfiling == None: singleLayerProfiling = False
  ...
  if profiling:
    execTimeFromProfiling = (profilingTable[len(profilingTable)-1]['dur'] + profilingTable[0]['dur'] + profilingTable[1]['dur']) / 1000000

    #If we are profiling single Layer execution times, them we don't consider session initializzations
    if singleLayerProfiling:
      execTimeFromProfiling = profilingTable[len(profilingTable)-1]['dur'] / 1000000
